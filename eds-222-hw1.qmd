...
::: callout-warning
There's an implicit assumption about file organization in the code above. What is it? How can you make the code work? Must create a data folder, and the assumption is that your project is in its own repo folder. Loading the data into the data folder makes the code work.
:::

1.  These dataframes have one row per pollution observation. How many pollution records are in each dataset?

There are 5488 pollution crowdsourced pollution records and 1960 government pollution records.

2.  Each monitor is located at a unique latitude and longitude location. How many unique monitors are in each dataset?

There are 14 unique monitors in the crowdsourced data set and 5 unique monitors in the govt dataset.

```{r}
# Find unique values
unique_govt_values <- govt |> 
  group_by(longitude, latitude) |> 
  summarise(list(cur_group_id()))          # List out the unique numeric identifier for each group 

unique_crowdsourced_values <- crowdsourced |> 
  group_by(longitude, latitude) |> 
  summarise(list(cur_group_id()))
```
...
The population is the total exposure of 11.1 million people in Lahore, Pakistan over the given time period, who are susceptible increased respiratory and cardiovascular health risks, ranking Lahore as the 12th most polluted city in the world. 

2.  What are the *samples* in this setting? Please be precise.

The samples are the pollution level monitors that are crowdsourced and employed by the Lahore government over the given time period. The 2 samples are the areas where the monitors are being installed, by crowdsource or government. Both samples represent observations from November 4th, 2018 to November 30th 2019
...
There is definitely a convenience bias by choosing the location of the government employed monitors where the air quality is not as bad. It may be true that it is easier for the government to set up these monitors in areas with less air pollution, or it is that this was an example of undercoverage/confirmation bias in failing to provide a representative sample of the wider city pollution exposure. There is certainly a selection bias in those who volunteer to use their homes as monitors. It is likely that those who care more about the issue or have the means to volunteer WILL volunteer, making the sample not representative of the whole population. Moreover, it is likely that the crowdsource data came from citizens who are educated and have the means of uploading this data to a computer server. 
...
crowdsrced_mean <- mean(crowdsourced$PM)
crowdsrced_min <- min(crowdsourced$PM)
crowdsrced_max <- max(crowdsourced$PM)
```

For the government data, the sample mean is `r govt_mean` $\mu g/m^3$. The government sample minimum value is `r govt_min` $\mu g/m^3$. The government sample max value is `r govt_max` $\mu g/m^3$.

For the crowdsourced data, the sample mean is `r crowdsrced_mean` $\mu g/m^3$. The crowdsourced sampled minimum value is `r crowdsrced_min` $\mu g/m^3$. The max value for the crowdsourced sample is `r crowdsrced_max` $\mu g/m^3$.

2.  Discuss any key differences that you see between these two samples.

The key differences between these two samples are that the statistic mean, min, and max values in the crowdsourced sample are overall much higher. There are also many more observations in the crowdsourced sample compared to the government sample. The entire distribution gives smaller PM levels, as a result of the government data.

3.  Are the differences in mean pollution as expected, given what we know about the sampling strategies?

Yes, the differences in mean pollution are expected given the bias we predicted before analyzing the data. With the government employing their own pollution monitors, we predicted that perhaps the values would be lower to make the levels and urgency of the pollution not appear as prominent. This turned out to be true with the lower overall mean in government samples.
...
```{r}
govt <- govt |> mutate("sample_type" = "government")                             # Preparing to join, creating a sample type column

crowdsourced <- crowdsourced |> mutate("sample_type" = "crowdsourced")           # Same sample type key

full_sample_join <- full_join(govt, crowdsourced)                                # Join both 

ggplot(full_sample_join) +
  geom_point(aes(x = longitude, y = latitude, color = sample_type)) +
  theme_light() +
  labs(x = "Longitude", 
       y = "Latitude",
       title = "Air Pollution in Lahore, Pakistan",
       color = "Sample Type")
...
Relative to crowd-sourced monitors, the government monitor locations are highly concentrated in the same general area of latitude and longitude. Moreover, the political bias may be that the government purposely ignores sampling for pollution in the rest of Lahore. The crowd sourced monitors are more abundant in quantity but also in location spread.
...
```{r}
stratified <- full_sample_join |>  
  mutate(lat_round = round(latitude, digits = 2))                  # The round tells us that there are 5 groups of latitudes


strat_slice <- stratified |> 
  slice_sample(n = 200, by = lat_round, replace = FALSE)        # We want to sample 200 from 5 groups to add up to 1000
```
...
```{r}
estimated_mean_rand <- df_sample |> summarise(mean(PM))       # 62.666 

estimated_mean_strat <- strat_slice |> summarise(mean(PM))        # 66.95

estimated_mean_corrupt <- full_sample_join |> summarise(mean(PM))        # 62.1603
```

The stratified random sample seems to be the closest to the satellite data. Since the strata were based on the non uniform sample employed by the government, the Prime Minister can employ a greater spread of monitors of specifically government type, or crowdsourced data in greater areas as well. This way, he can more evenly randomize the stratified samples over a greater area or time frame. This would still rely on the crowdsourced data more heavily than the government because they have fewer existing samples. 
