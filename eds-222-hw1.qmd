...
Must create a data folder, and the assumption is that your project is in its own repo folder. Loading the data into the data folder makes the code work.
:::
...
The population is the total exposure of 11.1 million people in Lahore, Pakistan who are susceptible increased respiratory and cardiovascular health risks, ranking Lahore as the 12th most polluted city in the world.

2.  What are the *samples* in this setting? Please be precise.

The samples are the pollution level monitors that are crowdsourced and employed by the Lahore government. 
...
The key differences between these two samples are that the statistic mean, min, and max values in the crowdsourced sample are overall much higher. There are also many more observations in the crowdsourced sample compared to the government sample. 
...
:::
```{r}
gov_sf <- st_as_sf(govt, coords = c('longitude', 'latitude'))

crowd_sf <-st_as_sf(crowdsourced, coords = c('longitude', 'latitude'))
